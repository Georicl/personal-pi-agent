...
Scientific Figure 是 Personal Pi 的第一个学术工作流。它不是固定图表模板，而是一条由 Pi 在一次用户请求中自动完成的通用流程：理解问题、检查数据、选择绘图方法、生成、确定性验证、必要时修订，并把每一版图片送到 GUI 右侧预览。
...
用户提示 + 数据文件
        │
        ▼
Pi Skill: scientific-figure
        │ 规范问题、图形语法与输出要求
        ▼
scientific_figure_inspect_data
        │ 形状、列、类型、缺失、范围、小样本
        ▼
统计检验？ ── 是 ──► 说明方法并等待用户确认
        │ 否/已确认
        ▼
scientific_figure_render
...
打包目录 `Resources/ScientificFigure/` 包含：

| 文件 | 职责 |
|---|---|
| `skill/SKILL.md` | 工作方法、统计边界、循环和完成条件 |
| `skill/references/` | 包路由、图形配方、质量清单 |
| `extension.js` | Pi tools、确认交互、官方文档检索、artifact details |
| `runner.py` | 数据读取、受控命名空间、导出和确定性验证 |
| `pyproject.toml` / `uv.lock` | 可复现 Python 依赖环境 |

Pi 启动参数会显式附加：

~~~text
--extension <ScientificFigure>/extension.js
--skill <ScientificFigure>/skill/SKILL.md
~~~

因此用户既可以自然语言提出绘图要求，也可以从命令面板调用 `/skill:scientific-figure`。

## 6. Extension tools

| Tool | 输入重点 | 结果 details |
|---|---|---|
| `scientific_figure_capabilities` | 无 | `personalPiFigureCapabilities` |
| `scientific_figure_inspect_data` | `dataPath` | `personalPiFigureInspection` |
| `scientific_figure_library_docs` | `library`, optional `topic` | 官方 URL、来源类型 |
| `scientific_figure_render` | `title`, `code`, paths, figure/version, dimensions, DPI, optional confirmed statistics | `personalPiFigureArtifact` |

文档检索只访问 Skill 参考表中列明的官方文档源；网络不可用时返回打包参考，不虚构 API。

## 7. Artifact manifest

`scientific_figure_render` 在 `tool_execution_end.result.details.personalPiFigureArtifact` 返回：
...
Swift 解析层只接受这一结构化字段，不从助手文本提取路径。`id` 由产物绝对路径稳定派生以避免跨项目冲突；`FigureArtifactStore` 用 `id` 去重，并按 `figureId + Session + CWD` 聚合版本和恢复最近图片。
...
默认版本目录只有图片。`source.py`、`request.json`、`validation.json` 和 `runtime.log` 在成功或失败后都会清理；只有用户明确要求，或 Settings 中明确启用 `scientificFigure.keepWorkFiles`，才保留这些工作文件。
...
Settings → Advanced runtime → Scientific figures 管理两个非 Pi 原生、由本 Extension 读取的嵌套键：
...
`pythonPath` 为空时，Personal Pi 使用 uv 在 `~/.pi/agent/environments/scientific-figure/` 创建受 `uv.lock` 约束的环境。Project 设置可以覆盖 Global。自定义 Python 必须自行提供锁文件中的依赖。
...
专用检查在临时 uv 环境中读取 CSV，生成三格式文件，核对 300 DPI 与物理尺寸、默认清理策略，并启动真实 Pi RPC 验证 Extension marker 与 `/skill:scientific-figure`。
